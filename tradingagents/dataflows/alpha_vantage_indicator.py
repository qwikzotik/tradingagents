...
# Mapping from internal indicator names to (display_name, required_series_type)
...
# Mapping from internal indicator name to Alpha Vantage CSV column name
_CSV_COLUMN_MAP = {
...
def _fetch_indicator_data(indicator, symbol, interval, time_period, series_type):
    """Dispatch the correct Alpha Vantage API call for a given indicator.

    Returns the raw CSV string, or a formatted result string for indicators
    that don't have a direct API equivalent (e.g. VWMA).
    """
    if indicator == "close_50_sma":
        return _make_api_request("SMA", {
            "symbol": symbol, "interval": interval,
            "time_period": "50", "series_type": series_type, "datatype": "csv",
        })
    elif indicator == "close_200_sma":
        return _make_api_request("SMA", {
            "symbol": symbol, "interval": interval,
            "time_period": "200", "series_type": series_type, "datatype": "csv",
        })
    elif indicator == "close_10_ema":
        return _make_api_request("EMA", {
            "symbol": symbol, "interval": interval,
            "time_period": "10", "series_type": series_type, "datatype": "csv",
        })
    elif indicator in ("macd", "macds", "macdh"):
        return _make_api_request("MACD", {
            "symbol": symbol, "interval": interval,
            "series_type": series_type, "datatype": "csv",
        })
    elif indicator == "rsi":
        return _make_api_request("RSI", {
            "symbol": symbol, "interval": interval,
            "time_period": str(time_period), "series_type": series_type, "datatype": "csv",
        })
    elif indicator in ("boll", "boll_ub", "boll_lb"):
        return _make_api_request("BBANDS", {
            "symbol": symbol, "interval": interval,
            "time_period": "20", "series_type": series_type, "datatype": "csv",
        })
    elif indicator == "atr":
        return _make_api_request("ATR", {
            "symbol": symbol, "interval": interval,
            "time_period": str(time_period), "datatype": "csv",
        })
    elif indicator == "vwma":
        return None  # No direct API; caller handles this case
    else:
        return None


def _parse_indicator_csv(data, indicator, before, curr_date_dt):
    """Parse Alpha Vantage CSV response and extract values in the date range.

    Returns a list of (datetime, value_str) tuples sorted by date, or an error string.
    """
...
def get_indicator(
    ...
    try:
        # Handle VWMA special case (no direct API)
        if indicator == "vwma":
            return (
                f"## VWMA (Volume Weighted Moving Average) for {symbol}:\n\n"
                "VWMA calculation requires OHLCV data and is not directly available from Alpha Vantage API.\n"
                "This indicator would need to be calculated from the raw stock data using volume-weighted price averaging.\n\n"
                + AV_INDICATOR_DESCRIPTIONS.get("vwma", "No description available.")
            )

        data = _fetch_indicator_data(indicator, symbol, interval, time_period, series_type)
        if data is None:
            return f"Error: Indicator {indicator} not implemented yet."

        result_data = _parse_indicator_csv(data, indicator, before, curr_date_dt)

        # _parse_indicator_csv returns a string on error
        ...
        result_str = (
            ...
            + ind_string
            + "\n\n"
            + AV_INDICATOR_DESCRIPTIONS.get(indicator, "No description available.")
        )

        return result_str

    except (KeyError, ValueError, RequestException, AlphaVantageRateLimitError) as e:
